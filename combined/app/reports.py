...
API_CONTRACT_ROWS = [
    ["User Action", "Swagger Endpoint", "Backend Flow", "Data Impact"],
    ["Create short link", "POST /shorten", "main.shorten() validates CreateShortUrlRequest, then calls UrlService.create(target_url, custom_slug).", "INSERT into urls: slug, target_url, created_at. Duplicate custom endpoint returns 409."],
    ["Open short link", "GET /{slug}", "main.redirect() calls UrlService.resolve(slug), then returns HTTP 307 redirect to target_url.", "UPDATE urls: clicks = clicks + 1, last_accessed_at = now."],
    ["View analytics", "GET /stats/{slug}", "main.stats() calls UrlService.get(slug) and returns UrlStatsResponse.", "SELECT urls row: slug, target_url, created_at, clicks, last_accessed_at."],
    ["Run SDLC workflow", "POST /agent/scenarios/{scenario}/run", "main.run_scenario() calls SdlcOrchestrator.start() for greenfield, brownfield, or ambiguous.", "Creates in-memory run context with node_status, decision_lineage, approvals, and metrics."],
    ["Approve checkpoint", "POST /agent/runs/{run_id}/approve/{node_id}", "main.approve_run() records approval and resumes orchestrator execution.", "Approval is stored in run context and audit log; release can complete only after approval."],
    ["Download evidence", "GET /agent/runs/{run_id}/report", "main.workflow_report() passes run context to reports.build_sdlc_pdf_from_run().", "Generates PDF evidence with stages, metrics, decisions, and governance proof."],
]

DATA_MODEL_ROWS = [
    ["Field", "Where It Appears", "Purpose"],
    ["slug", "urls table, API path /{slug}", "Unique short endpoint chosen by user or generated by the system."],
    ["target_url", "urls table", "Original long URL that redirect sends users to."],
    ["created_at", "urls table and stats response", "Timestamp proving when the endpoint was created."],
    ["clicks", "urls table and stats response", "Analytics counter incremented on every redirect."],
    ["last_accessed_at", "urls table and stats response", "Timestamp updated on last redirect."],
    ["run_id", "workflow API/report", "Trace identifier for one SDLC workflow execution."],
    ["node_status", "workflow response/report", "Stage-by-stage state of requirements, architecture, tests, docs, release, etc."],
    ["metrics", "workflow response/report", "Reliability evidence: success rate, retries, rollback count, latency, MTTR."],
]
SCENARIO_PACKETS = {
    "overall": {
...
def build_analysis_pdf(scenario: str = "overall") -> Path:
    ...
    story.extend(_api_contract_section(styles))
    story.extend(
        [
...
def _api_contract_section(styles: dict[str, ParagraphStyle]) -> list[Any]:
    return [
        _para("How The /docs API Works Behind The UI", styles["h2"]),
        _para(
            "The evaluator can verify each UI action in Swagger at /docs. The UI is not fake; it calls these FastAPI routes, which call service methods and update SQLite state.",
            styles["body"],
        ),
        _table(API_CONTRACT_ROWS, [1.05 * inch, 1.35 * inch, 2.25 * inch, 2.1 * inch], styles),
        Spacer(1, 8),
        _para("Data Model Evidence", styles["h2"]),
        _table(DATA_MODEL_ROWS, [1.3 * inch, 1.75 * inch, 3.7 * inch], styles),
        Spacer(1, 8),
        _para("Concrete API Examples", styles["h2"]),
        _bullet("POST /shorten with {url: 'https://www.schwab.com', custom_slug: 'schwab-demo'} returns a short_url and persists the endpoint.", styles["body"]),
        _bullet("GET /schwab-demo returns a 307 redirect and increments clicks plus last_accessed_at.", styles["body"]),
        _bullet("GET /stats/schwab-demo returns click analytics used by the UI analytics panel.", styles["body"]),
        _bullet("POST /agent/scenarios/greenfield/run starts the governed SDLC workflow and returns node_status, metrics, and decision lineage.", styles["body"]),
        _bullet("POST /agent/runs/{run_id}/approve/release records the human checkpoint and lets release readiness complete.", styles["body"]),
    ]
