...
BLUEPRINTS: dict[ScenarioName, ScenarioBlueprint] = {
    ScenarioName.greenfield: ScenarioBlueprint(
        ...
        ai_interactions=[
            {
                "task": "G-API",
                "prompt_intent": "Ask AI for a minimal FastAPI route structure with Pydantic request/response schemas.",
                "constraints": ["No secrets", "No external services required", "Keep service layer testable"],
                "ai_suggestion": "Create /api/links, /r/{code}, /api/links/{code}/stats, and /health.",
                "engineer_action": "edited",
                "rationale": "Kept route set but moved business rules into UrlService for maintainability.",
            },
            {
                "task": "G-TEST",
                "prompt_intent": "Ask AI to propose edge-case tests for redirect controls.",
                "constraints": ["Tests must be deterministic", "No network calls"],
                "ai_suggestion": "Test expired link, click limit, duplicate endpoint, and stats increment.",
                "engineer_action": "accepted",
                "rationale": "Suggestions matched product risks and were implemented with local TestClient.",
            },
        ],
        engineer_decisions=[
    ...
    ScenarioName.brownfield: ScenarioBlueprint(
        ...
        ai_interactions=[
            {
                "task": "B-IMPACT",
                "prompt_intent": "Ask AI to inspect impact of adding expiry to an existing URL shortener.",
                "constraints": ["Preserve existing API", "No database-breaking migration", "Keep redirect analytics accurate"],
                "ai_suggestion": "Add expires_at to create request and check it in resolve before incrementing clicks.",
                "engineer_action": "edited",
                "rationale": "Added max_clicks and disabled handling too, because same control point handles reliability policy.",
            },
            {
                "task": "B-TEST",
                "prompt_intent": "Ask AI for regression tests after brownfield control changes.",
                "constraints": ["Existing create and redirect tests must still pass"],
                "ai_suggestion": "Test expired links, click-limit exceeded links, and unchanged stats path.",
                "engineer_action": "accepted",
                "rationale": "Directly validates backward compatibility and new failure modes.",
            },
        ],
        engineer_decisions=[
    ...
    ScenarioName.ambiguous: ScenarioBlueprint(
        ...
        ambiguity_notes=[
            "Safer could mean abuse detection, password protection, expiry, or domain allowlists.",
            "Smarter could mean analytics, campaign tagging, QR codes, or routing rules.",
            "Success metrics and compliance constraints are not stated.",
        ],
        ...
        tasks=[
            ...
            {"id": "A-GATE", "task": "Require engineer sign-off before implementation", "depends_on": ["A-PLAN"]},
        ],
        ai_interactions=[
            {
                "task": "A-CLARIFY",
                "prompt_intent": "Ask AI to identify ambiguity in a vague product requirement.",
                "constraints": ["Do not invent scope", "Return questions and assumptions separately"],
                "ai_suggestion": "Ask about abuse model, analytics depth, retention, privacy, and release priority.",
                "engineer_action": "accepted",
                "rationale": "Questions are directly tied to risk and implementation choices.",
            },
            {
                "task": "A-PLAN",
                "prompt_intent": "Ask AI for implementation options after assumptions are stated.",
                "constraints": ["Prefer smallest safe prototype", "Highlight rejected options"],
                "ai_suggestion": "Start with expiry and click limits; defer ML abuse detection.",
                "engineer_action": "edited",
                "rationale": "Engineer narrowed implementation to deterministic controls suitable for this prototype.",
            },
        ],
        engineer_decisions=[
        ...
        quality_gates=[
            ...
            {"name": "scope_control", "status": "passed", "evidence": "Rejected vague ML/abuse claims without data"},
        ],
        risks=[
...
class EngineeringEvidenceService:
    ...
    def execute(
        ...
        status = self._status_for(scenario, engineer_signoff)
        evidence = {
            "run_id": run_id,
            ...
            "normalized_problem": self._normalize(requirement, blueprint),
            "ambiguity_notes": blueprint.ambiguity_notes,
            "task_decomposition": blueprint.tasks,
            "codebase_reasoning": {
                "impacted_components": blueprint.impacted_components,
                "data_flow": blueprint.data_flow,
            },
            "ai_assisted_execution": blueprint.ai_interactions,
            "traceability": self._traceability(blueprint),
            ...
            "quality_gates": self._quality_gates(blueprint, engineer_signoff),
            "risks": blueprint.risks,
            "artifacts": blueprint.artifacts,
            "assumptions": blueprint.assumptions,
            "limitations": blueprint.limitations,
            "final_summary": self._summary(blueprint, status),
    ...
    def _status_for(self, scenario: ScenarioName, engineer_signoff: bool) -> str:
    ...
    def _normalize(self, requirement: str, blueprint: ScenarioBlueprint) -> str:
    ...
    def _traceability(self, blueprint: ScenarioBlueprint) -> list[dict[str, str]]:
        return [
            {
                "task": item["task"],
                "generated": item["ai_suggestion"],
                "engineer_action": item["engineer_action"],
                "rationale": item["rationale"],
            }
            for item in blueprint.ai_interactions
        ]

    def _quality_gates(self, blueprint: ScenarioBlueprint, engineer_signoff: bool) -> list[dict[str, object]]:
        gates = list(blueprint.quality_gates)
        gates.append(
            {
                "name": "engineer_ownership",
                "status": "passed" if engineer_signoff else "waiting",
                "evidence": "Engineer sign-off recorded" if engineer_signoff else "Engineer sign-off required before final acceptance",
            }
        )
        return gates

    def _summary(self, blueprint: ScenarioBlueprint, status: str) -> str:
        return (
            f"{blueprint.title} produced a reviewable engineering package with requirement analysis, "
            f"task sequencing, AI assistance traceability, quality gates, risks, and limitations. Current status: {status}."
        )
